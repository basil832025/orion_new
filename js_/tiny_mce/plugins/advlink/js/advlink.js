...
function insertAction() {
	...
	var target = getSelectValue(f, 'targetlist');

	// если href пустой – просто удаляем ссылку / ничего не делаем
	if (!href) {
		tinyMCEPopup.close();
		return;
	}

	// берём то, что было выделено в редакторе
	var selectedHtml = ed.selection.getContent({ format: 'html' });

	// если ничего не выделено – текстом ссылки сделаем сам URL
	if (!selectedHtml) {
		selectedHtml = href;
	}

	// собираем <a> вручную, без всяких #mce_temp_url#
	var attrs = ' href="' + href + '"';
	if (target && target !== '_self') {
		attrs += ' target="' + target + '"';
	}

	// вставляем/заменяем содержимое
