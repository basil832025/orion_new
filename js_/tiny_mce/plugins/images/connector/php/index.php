<?php
include 'config.php';
/** Сделаем ответы «чистыми» и без двойной компрессии */
ini_set('display_errors', '0');
error_reporting(E_ALL & ~E_DEPRECATED & ~E_NOTICE & ~E_WARNING);
ini_set('zlib.output_compression', '0');
if (function_exists('header_remove')) { @header_remove('Content-Encoding'); }
while (ob_get_level() > 0) { @ob_end_clean(); } // убрать неожиданные буферы
//echo 'trtrt'. DIR_IMAGES;exit;
class TinyImageManager {
	...
	public function __construct() {
		...
		header('Content-Type: text/html; charset=utf-8'); // тут лучше оставить text/html

		if (!isset($_SESSION['tiny_image_manager_path'])) {
			$_SESSION['tiny_image_manager_path'] = '';
		}

		$this->ALLOWED_IMAGES = ['jpeg','jpg','gif','png'];
		$this->ALLOWED_FILES  = ['doc','docx','ppt','pptx','xls','xlsx','mdb','accdb','swf','zip','rar','rtf','pdf','psd','mp3','wma'];

		$this->dir = [
			'images' => realpath(DIR_IMAGES) ?: DIR_IMAGES,
			'files'  => realpath(DIR_FILES)  ?: DIR_FILES,
		];
	...
	//	p($this->dir);
		...
		foreach ($innerDirs as $v) {
			...
			if(isset($v['childs'])) {
				...
				if($soc>0 && !empty($currentDirArr[$soc]) && $currentDirArr[$soc]==$v['name']) {
					$folderAct = 'folderAct';
				} else {
					$folderAct = '';
				}
