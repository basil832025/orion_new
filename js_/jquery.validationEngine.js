...
jQuery.fn.validationEngine = function(settings) {
	if($.validationEngineLanguage){					// IS THERE A LANGUAGE LOCALISATION ?
		...
		allRules = {"required":{    			  // Add your regex rules here, you can take telephone as an example
						...
						"noSpecialCaracters":{
							"regex":"/^[_0-9a-zA-Zа-яА-Я]+$/",
							"alertText":"Специальные символы и пробелы запрещены!"},	
						"onlyLetter":{
							"regex":"/^[а-яА-Я]+$/",
...
// ДЛЯ АДМИНКИ ПРОВЕРКА
	$('.ajax_send').on('click',  function(caller){   // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
obj_valid=$("#form_edit_form");
  		if(submitValidation(obj_valid) == false){
			if (settings.success){
...
$("form").on("submit", function(caller){   // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
	...
	$(this).find("[type=checkbox]").on("click", function(caller){loadValidation(this)})
	
	var buildPrompt = function(caller,promptText) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
		var divFormError = document.createElement('div')
		var formErrorContent = document.createElement('div')
		var arrow = document.createElement('div')
		name_atr = $(caller).attr("name");
        name_atr=name_atr.replace(/\[/g,"");
        name_atr=name_atr.replace(/\]/g,"");
     	$(divFormError).addClass(name_atr)
		...
		function _required(caller,rules){   // VALIDATE BLANK FIELD
			...
			if (callerType == "text" || callerType == "password" || callerType == "textarea"){
				...
				if(!$(caller).val()){
                   ...
                   // alert(caller)
                     $(caller).stop()
                     $(caller).animate({margin: "0 0 0 10px"}, 100)
                     $(caller).animate({margin: "0 20px 0 0"}, 100);
                     $(caller).animate({margin: "0 0 0 10px"}, 100)
                     $(caller).animate({margin: "0 20px 0 0"}, 100);
                      $(caller).animate({margin: "0 0 0 10px"}, 100)
                     $(caller).animate({margin: "0 20px 0 0"}, 100);
                   
                     $(caller).animate({margin: "0 0 0 0"}, 100);
                 $(caller).addClass("required");
				}	
			}
	...
	var closePrompt = function(caller) {	// CLOSE PROMPT WHEN ERROR CORRECTED
		closingPrompt = $(caller).attr("name")
      	name_atr = closingPrompt;
        name_atr=name_atr.replace(/\[/g,"");
        closingPrompt=name_atr.replace(/\]/g,"");
		$("."+closingPrompt).fadeTo("fast",0,function(){
		  $("."+closingPrompt).remove();
          $(caller).removeClass("required");
