...
function get_player($field,$id)
{
 ...
 //r.rt_id_'.$num_player.'_beg as reiting,
  $sql='select  name,  tp.beg_reiting ,p.reiting_ukraine,
  group_num, pl_num_grp'.$num_player.'  as pl_num,type_game, olimp16_num,r.set_1,r.set_2      
  from '.T_REITING.' r,'.T_PLAYERS.' p, bs_turnirplayers tp  where  tp.player_id=p.id AND p.id=r.'.$field.' and  r.id='.$id.' and tp.turnir_id='.$turnir_id;
  //s($sql);
/*    $sql='select  name, p.reiting,p.reiting_ukraine,
  group_num, pl_num_grp'.$num_player.'  as pl_num,type_game, olimp16_num,r.set_1,r.set_2      
  from '.T_REITING.' r,'.T_PLAYERS.' p  where p.id=r.'.$field.' and  r.id='.$id;*/
  $aRes = db_row($sql); 
  $virt_name_player = 'Чекаємо гравця';
  if (!empty($aRes)) {
      ...
      $virt_name_player = ($aRes['type_game'] == 2 && !empty($aRes['olimp16_num'])) ? 'Очікується гравець (' . $aRes['olimp16_num'] . ')' : '';
      $virt_name_player = ($aRes['type_game'] == 1) ? 'Група ' . $aRes['group_num'] . ' гравець ' . $aRes['pl_num'] : $virt_name_player;
      $txt_FNTU = $aRes['reiting_ukraine'] > 0 ? '/<span class="f14NewDis">' . $aRes['reiting_ukraine'] . '</span>' : '/0.00';

      $name_reiting = !empty($aRes['name']) ? '<div class="p-1 f14NewDis' . $is_win_class . '">' .$aRes['name']  . '<br> <span class="f14NewDis">' . round($aRes['beg_reiting']) . '</span>' . $txt_FNTU . '</div>'
          : '';
  }
  ...
  function sql_raschet($turnir_id, $etap_id,$this_aResults,$group_num)
    { global $mesto_in_grp,$aMestaPlayersGrp;
       ...
       $sql = 'SELECT p.name,tp.player_id,groups,grp_num,grp_win_set,grp_lose_set,grp_mesto,grp_ochki,case when reiting>0 then reiting else start_reiting end as beg_reit
 FROM '.T_ETAPS_PLAYER_MESTA.' tp, '.T_PLAYERS.' p where  turnir_id='.$turnir_id.' and etap_id='.$etap_id.' and groups='.$group_num.' and p.id=tp.player_id 
...
//s($sql);
  ...
  /*    function obrPoSets_old($aTempPlayers,$aAll=array())
             ...
             foreach ($aTempPlayers2 as $key2 => $aPlay2)
                {
                    if ($key2<>$key) 
                    {// кто победил между 2 игроками
                      $id_win =  $this_aResults[$group_this][$play_this][$aPlay2['grp_num']]['win'];
     ...
     function all_results ($turnir_id,$etap_id)
   {
    ...
    {
        $this_aResults[$aRec['group_num']][$aRec['pl_num_grp1']][$aRec['pl_num_grp2']]['first_res'] = $aRec['set_1'];
        $this_aResults[$aRec['group_num']][$aRec['pl_num_grp1']][$aRec['pl_num_grp2']]['second_res'] = $aRec['set_2'];
        if ($aRec['set_1']>$aRec['set_2'])
            $this_aResults[$aRec['group_num']][$aRec['pl_num_grp1']][$aRec['pl_num_grp2']]['win'] = $aRec['pl_num_grp1'];
         else     
           $this_aResults[$aRec['group_num']][$aRec['pl_num_grp1']][$aRec['pl_num_grp2']]['win'] = $aRec['pl_num_grp2'];
       
          // теперь перевоачиваем для нижней части таблицы
        $this_aResults[$aRec['group_num']][$aRec['pl_num_grp2']][$aRec['pl_num_grp1']]['first_res'] = $aRec['set_2'];
        $this_aResults[$aRec['group_num']][$aRec['pl_num_grp2']][$aRec['pl_num_grp1']]['second_res'] = $aRec['set_1'];
     if ($aRec['set_2']>$aRec['set_1'])
            $this_aResults[$aRec['group_num']][$aRec['pl_num_grp2']][$aRec['pl_num_grp1']]['win'] = $aRec['pl_num_grp2'];
         else     
           $this_aResults[$aRec['group_num']][$aRec['pl_num_grp2']][$aRec['pl_num_grp1']]['win'] = $aRec['pl_num_grp1'];
...
while ($n<=$cnPeople) 
{
   ...
   //  s('nnnnnnnnnn===='.$n);
    ...
    { 
       $player_id=   $aGroupMesto[$numGrpZmeyki][$mestowork]['player_id'];
