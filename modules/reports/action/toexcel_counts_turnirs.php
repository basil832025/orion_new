...
$name_period = ' з '.date_for_firebird_format($dbeg).' по '.date_for_firebird_format($dend). ' '.$txtCity. ' '.$txtClub;

$objWorkSheet = $objPHPExcel->createSheet(0); //Setting index when creating
get_zagolovok_counts_turnits($objWorkSheet,'Діти-молодша',$name_period);
...
{
    set_data_counts_turnits($objWorkSheet,$aUsers,$aUsersNO);
}
$objWorkSheet = $objPHPExcel->createSheet(1); //Setting index when creating
get_zagolovok_counts_turnits($objWorkSheet,'Діти середні',$name_period);
...
get_zagolovok_counts_turnits($objWorkSheet,'Діти старші',$name_period);
...
get_zagolovok_counts_turnits($objWorkSheet,'Діти-ранок',$name_period);
...
get_zagolovok_counts_turnits($objWorkSheet,'ШВСМ',$name_period);
...
get_zagolovok_counts_turnits($objWorkSheet,'Дорослі',$name_period);
...
get_zagolovok_counts_turnits($objWorkSheet,'Загальна',$name_period);
...
header('Content-Disposition: attachment;filename="'.'Кількість_турнірів'.DATE('dmy_Hi').'.xls"');
